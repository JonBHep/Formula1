...
internal class Driver : IComparable<Driver>
{
    ...
    {
        get => $"{_forenames} {_surname}";
    }

    internal string SortingName
    {
        get => $"{_surname} {_forenames}";
    }
    ...
    // TODO change some of the DateTime to DateOnly
    ...
    internal DateTime RuntimeFirstWinDate { get; set; }
    
    // internal int RuntimeFirstChampYear { get; set; }
    ...
    internal Core.CauseOfDeath HowDied
    {
        get => _causeOfDeath;
    ...
    internal DateTime DeathDate
    {
        get => _deathDate;
        set => _deathDate = value;
    }

    internal DateTime CeiDate
    {
        get => _ceiDate;
        set => _ceiDate = value;
    }

    internal string DeathMode
    {
        get => _deathModeText;
        set => _deathModeText = value;
    }
    ...
    int IComparable<Driver>.CompareTo(Driver? other)
    {
        if (other is { } autre)
        {
            ...
            int q = string.Compare(_surname, autre._surname, true, Core.CultureUk);
            if (q == 0)
            {
                q = string.Compare(_forenames, autre._forenames, true, Core.CultureUk);
            }
    ...
    public bool Complete
    {
        get
        {
            bool replete = !string.IsNullOrWhiteSpace(_surname);
            if (string.IsNullOrWhiteSpace(_forenames))
            {
                replete = false;
            }

            if (string.IsNullOrWhiteSpace(_deathModeText))
            {
                replete = false;
            }

            if (_birthDate < Core.DateBase)
            {
                replete = false;
            }
            ...
            if ((_deathDate > Core.DateBase) && (HowDied == Core.CauseOfDeath.Unknown))
            {
                replete = false;
            }
    ...
    internal string WikiLinkString
    {
        get { return $"https://en.wikipedia.org/wiki/ {FullName}"; }
    }
    ...
    internal string DisplayAge()
    {
        ...
        if (_deathDate < Core.DateBase)
        {
            return $"Alive aged {age}";
        }
        else
        {
            return $"Died aged {age}";
        }
    }
    ...
    internal bool NotDiedBefore(DateTime Q)
    {
        ...
        {
            return true;
        } // known to have died later than this
    ...
    internal string TimeLeft(DateTime Q)
    {
        ...
        {
            return string.Empty;
        } // don't know when he died, or he's still alive
    ...
    internal void AppendFateGlyph(ref TextBlock t)
    {
        ...
        if ((HowDied == Core.CauseOfDeath.RacePracticeOrTestingAccident) ||
            (HowDied == Core.CauseOfDeath.RacingAccident))
        {
            r = new Run()
            {
                FontFamily = new FontFamily("Webdings"), Text = Core.GlyphFatalAccident, FontSize = 16
        ...
        else if ((BirthDate > Core.DateBase) && !(DeathDate > Core.DateBase)) // not died
        {
            r = new Run()
            {
                FontFamily = new FontFamily("Webdings"), Text = Core.GlyphLiving, FontSize = 16
                , Foreground = Brushes.Blue
            }; // cat symbol (9 lives)
            if (CeiDate > Core.DateBase)
            {
                r.Foreground = Brushes.Chocolate;
            } // change colour of cat if driver's career was ended by injury
            ...
            {
                r.Foreground = Brushes.Chocolate;
            } // change colour of age if driver's career was ended by injury
    ...
    internal static string TextWorry(string txt)
    {
        ...
        if (txt.Contains("  "))
        {
            tw = "Double space in text";
        }

        if (txt.Contains("\""))
        {
            tw = "Double quotation mark in text";
        }
