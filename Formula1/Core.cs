...
{
    /*
         NB There have been F1 races that did not count towards the drivers’ championship and races that did count which were not F1 races, esp. in the early years.
        */
    ...
    internal static readonly CultureInfo CultureUk = CultureInfo.CreateSpecificCulture("en-GB");

    internal const int SpecialNumber = 5617;

    private static readonly Core Inst = new Core();
    ...
    internal enum CauseOfDeath
    {
        ...
        , Natural
        , RacingAccident
        , RacePracticeOrTestingAccident
        , OtherAccident
    };

    internal enum RankingHealth
    {
        Healthy
        , HasGaps
        , HasDuplicates
    ...
    private Core()
    {
        ...
        _firstChampionshipWins = new Dictionary<int, int>();
        LoadData();
    }

    internal static Core Instance =>
        Inst; // this static property allows global access to the single private instance of this class
    ...
    private readonly Dictionary<int, int>
        _firstChampionshipWins; // key is driver key, value is year of first championship win, aim is to save time on recalculating these
    ...
    internal static double LowWindowTop
    {
        ...
        {
            double
                scrY = System.Windows.SystemParameters
                    .PrimaryScreenHeight; // Locates the window towards bottom of screen to allow reading of webpage above
            return scrY - (LowWindowHeight + 30);
        }
    }

    internal static string RaceResultDescription(int position)
    {
        if (position <= 1000)
        {
        ...
        RaceResultConstants rrc = (RaceResultConstants) position;
        string r = "Undefined";
        switch (rrc)
        {
            ...
            {
                r = "Ret Retired (mechanical failure)";
                break;
            }
            case RaceResultConstants.RetFuel:
            {
                r = "Ret Retired (out of fuel)";
                break;
            }
            case RaceResultConstants.RetAccident:
            {
                r = "Ret Retired (accident / spun off)";
                break;
            }
            case RaceResultConstants.RetFatalDriver:
            {
                r = "FAT Fatal accident";
                break;
            }
            case RaceResultConstants.RetFatalOthers:
            {
                r = "Ret Retired (accident fatal to others)";
                break;
            }
            ...
            {
                r = "Ret Retired (driver unfit)";
                break;
            }
            case RaceResultConstants.RetUnexplained:
            {
                r = "Ret Retired (other reason)";
                break;
            }
        }
    ...
    internal enum RaceResultConstants
    {
        DidNotStart = 1001
        , RetMech = 1002
        , RetFuel = 1003
    ...
    internal void GetRecentDrivers(DateTime targetDate, out List<int> currentSeason, out List<int> last2Seasons)
    {
        ...
        foreach (Voiture c in _voitures.Values)
        {
            ...
            if ((c.RaceDate.Year == lastyear) ||
                (c.RaceDate.Year == targetDate.Year)) // driver has featured this season or last
            {
                for (int w = 0; w < 3; w++)
                {
                    if (c.DriverKey(w) > 0)
                    {
                        if (Instance.Drivers[c.DriverKey(w)].NotDiedBefore(targetDate))
                        {
                            occursLast[c.DriverKey(w)] = true;
                        }
                    }
                }
    ...
    internal void GetRecentConstructors(DateTime targetDate, out List<int> last12Months, out List<int> last24Months)
    {
        ...
        foreach (Voiture c in _voitures.Values)
        {
            DateTime raceDate = c.RaceDate;
            TimeSpan ago = (targetDate - raceDate);
            if (ago.TotalDays is < 730 and >= 0) // later than 2 years ago but not in the future
            {
    ...
    internal List<int> LivingDrivers(DateTime targetDate)
    {
        ...
        foreach (int k in conducteurs)
        {
            if (Instance.Drivers[k].NotDiedBefore(targetDate))
            {
                vivants.Add(k);
            }
        }
    ...
    internal RankingHealth GridHealth(int raceKey)
    {
        ...
        int gaps = 0;
        int dupes = 0;
        foreach (Voiture cr in entrants)
        {
            if (cr.GridPosition != SpecialNumber)
            {
                gridmax = Math.Max(gridmax, cr.GridPosition);
            }
        }
        ...
        if (gaps > 0)
        {
            health = dupes > 0 ? RankingHealth.HasGapsAndDupes : RankingHealth.HasGaps;
        }
        else
        {
            if (dupes > 0)
    ...
    internal List<int> GridFilledPlaces(int raceKey)
    {
        ...
        foreach (Voiture r in _voitures.Values)
        {
            if (r.RaceMeetingKey == raceKey)
            {
                if (r.GridPosition != SpecialNumber)
                {
                    filled.Add(r.GridPosition);
                }
            }
        }
    ...
    internal List<int> TeamsInRace(int raceKey)
    {
        ...
        {
            sortable.Add(new NamedItem(Constructors[x].Caption, x));
        }

        sortable.Sort();
        indices.Clear();
        foreach (NamedItem ni in sortable)
        {
            indices.Add(ni.Key);
        }
    ...
    internal List<int> DriversinTeamInRace(int raceKey, int teamKey)
    {
        ...
        foreach (Voiture r in _voitures.Values)
        {
            if (r.RaceMeetingKey == raceKey)
            {
                if (r.ConstructorKey == teamKey)
                {
                    ...
                    {
                        if (!indices.Contains(r.DriverKey(x)))
                        {
                            indices.Add(r.DriverKey(x));
                        }
                    }
                }
            }
        ...
        foreach (int x in indices)
        {
            sortable.Add(new NamedItem(Drivers[x].SortingName, x));
        }
    ...
    internal List<Tuple<int, int, int>> CompetitorRankings(int raceKey)
    {
        ...
        foreach (Voiture r in cars)
        {
            if (r.GridPosition != SpecialNumber)
            {
                gridPositions.Add(r.GridPosition);
            }
        }
        ...
        if (gridPositions.Count > 0)
        {
            ...
            foreach (Voiture r in cars)
            {
                ...
                if (p == SpecialNumber)
                {
                    last++;
                    p = last;
                } // any cars starting from the pit lane are added to the end of the grid
    ...
    {
        return dt.ToString("yyyyMMdd", CultureUk);
    }
    ...
    internal static DateTime DateFromCode(string cd)
    {
        ...
        int yi = int.Parse(ys, CultureUk);
        int mi = int.Parse(ms, CultureUk);
        int di = int.Parse(ds, CultureUk);
    ...
    internal void SaveData()
    {
        ...
        // Saving champion/year to save on recalculation time
        foreach (var key in _firstChampionshipWins.Keys)
        {
            sw.WriteLine($"CHAMP:{key}@{_firstChampionshipWins[key]}");
        }
        ...
        foreach (Circuit i in trx)
        {
            ...
            {
                System.Windows.MessageBox.Show($"Forgetting unreferenced circuit {i.CircuitSingleTitle(0)}"
                    , Jbh.AppManager.AppName, System.Windows.MessageBoxButton.OK
...
_firstChampionshipWins.Clear();
                ...
                if (lu is { } dat)
                {
                    ...
                    switch (clue)
                    {
                        ...
                        case "CHAMP:":
                        {
                            ...
                            int driver = int.Parse(part[0]);
                            int annum = int.Parse(part[1]);
                            _firstChampionshipWins.Add(driver, annum);
                            break;
                        }
    ...
    private void CleanUpSeasons()
    {
        ...
        foreach (int y in annees)
        {
            if (!_seasons.ContainsKey(y))
            {
                Season s = new Season()
                {
                    Anno = y, PointsAllocationSchemeForDrivers = new SeasonPointsAllocationScheme(0, 0, 0)
                    , PointsAllocationSchemeForConstructors = new SeasonPointsAllocationScheme(0, 0, 0)
                };
                _seasons.Add(y, s);
            }
        }
    ...
    {
        string q = f.ToString("0.00", CultureUk);
        while (q.EndsWith("0", StringComparison.OrdinalIgnoreCase))
        {
            q = q.Substring(0, q.Length - 1);
        }

        if (q.EndsWith(".", StringComparison.OrdinalIgnoreCase))
        {
            q = q.Substring(0, q.Length - 1);
        }
    ...
    {
        float p = 0;
        foreach (Voiture v in Instance.Voitures.Values)
        {
    ...
    internal static string WikiLinkPoints
    {
        get { return "https://en.wikipedia.org/wiki/List_of_Formula_One_World_Championship_points_scoring_systems"; }
    }
    ...
    private void UpdateDriverRuntimeProperties()
    {
        ...
        foreach (int k in _drivers.Keys)
        {
            ...
            firstWinDate = DateTime.MaxValue;
            ...
            foreach (Voiture v in _voitures.Values)
            {
                if (v.IncludesDriver(k))
                {
                    ...
                    if (!racesQualifiedKeys.Contains(rr))
                    {
                        racesQualifiedKeys.Add(rr);
                    }

                    RaceMeeting mtg = _races[rr];
                    DateTime rd = mtg.RaceDate;
                    if (!yearsRaced.Contains(rd.Year))
                    {
                        yearsRaced.Add(rd.Year);
                    }
                    ...
                    {
                        if (!racesStartedKeys.Contains(rr))
                        {
                            racesStartedKeys.Add(rr);
                        }
                        ...
                        if (v.RacePosition < 999)
                        {
                            if (!racesFinishedKeys.Contains(rr))
                            {
                                racesFinishedKeys.Add(rr);
                            }
                        }
            ...
            _drivers[k].RuntimeFirstWinDate = firstWinDate;
    ...
    internal List<Tuple<DateTime, int, bool>> DriversDiedSinceLastRace(int raceKey)
    {
        ...
        if (previousracedate < DateBase)
        {
            return returnList;
        } // this was the earliest race - no previous!
        ...
        foreach (Driver d in _drivers.Values)
        {
            ...
            {
                Tuple<DateTime, int, bool> tup
                    = new Tuple<DateTime, int, bool>(dd, d.Key, (d.HowDied != CauseOfDeath.Natural));
                returnList.Add(tup);
            }
        }
    ...
    internal List<Tuple<DateTime, int, bool>> DriversDiedBeforeNextRace(int raceKey)
    {
        ...
        foreach (RaceMeeting m in _races.Values)
        {
            ...
            if (rd > racedate)
            {
                if (rd < subsequentracedate)
                {
                    subsequentracedate = rd;
                }
            }
        }

        if (subsequentracedate == DateTime.MaxValue)
        {
            return returnList;
        } // this was the latest race - no subsequent!
    ...
    internal Tuple<int, int> ConstructorFirstLastYears(int team)
    {
        ...
        foreach (Voiture v in _voitures.Values)
        {
            if (v.ConstructorKey == team)
            {
                int y = v.RaceDate.Year;
                fy = Math.Min(fy, y);
                ly = Math.Max(ly, y);
            }
        }
    ...
    internal int MostRecentConstructorForDriver(int drv, DateTime asat)
    {
        ...
        foreach (Voiture v in _voitures.Values)
        {
            if (v.RaceDate < asat)
            {
                ...
                {
                    teamEvents.Add(new Tuple<DateTime, int>(v.RaceDate, v.ConstructorKey));
                }
            }
        }

        teamEvents.Sort();
        if (teamEvents.Count < 1)
        {
            return 0;
        } // e.g. for new driver
    ...
    internal string DriverShortName(int driverKey)
    {
        ...
        if (ct > 1)
        {
            shorty = _drivers[driverKey].FullName;
        }
    ...
    public static Tuple<int, int> AgeYearsDays(DateTime birth, DateTime death)
    {
        DateTime temp = birth;
        int years = 0;
        while (temp.Year < death.Year)
        {
            years++;
            temp = temp.AddYears(1);
        }

        if (temp.DayOfYear > death.DayOfYear)
        {
            years--;
            temp = temp.AddYears(-1);
        }
    ...
    internal List<Tuple<int, int>> OldestAndYoungestDriversOnRaceDay()
    {
        ...
        foreach (int dk in Drivers.Keys)
        {
            ...
            Tuple<double, int, int> q
                = new Tuple<double, int, int>(AgeDaysOnRaceDay(alphaomega.Item1, dk), alphaomega.Item1, dk);
            alphas.Add(q);
    ...
    internal int DriversAlive(int decade)
    {
        ...
        foreach (int k in q)
        {
            if (Drivers[k].DeathDate < DateBase)
            {
                a++;
            }
        }
    ...
    internal int DriversKilledRacing(int decade)
    {
        ...
        foreach (int k in q)
        {
            if (Drivers[k].HowDied == CauseOfDeath.RacingAccident)
            {
                a++;
            }
        }
    ...
    internal int DriversKilledRaceRelated(int decade)
    {
        ...
        {
            if ((Drivers[k].HowDied == CauseOfDeath.RacingAccident) ||
                (Drivers[k].HowDied == CauseOfDeath.RacePracticeOrTestingAccident))
            {
                a++;
            }
        }
    ...
    internal int DriversKilledAnyAccident(int decade)
    {
        ...
        foreach (int k in q)
        {
            if ((Drivers[k].HowDied == CauseOfDeath.RacingAccident) ||
                (Drivers[k].HowDied == CauseOfDeath.RacePracticeOrTestingAccident) ||
                (Drivers[k].HowDied == CauseOfDeath.OtherAccident))
            {
                a++;
            }
        }
    ...
    internal void UpdateRaceConsecutiveRaceRecords()
    {
        ...
        for (int j = 0; j < pisteKeys.Count; j++)
        {
            ...
            {
                int n = pisteKeys[j - 1];
                _races[m].ConWins.BringForward(_races[n].ConWins);
            }
    ...
    private float[] DetectMeetingPointsAwardScheme(RaceMeeting rm, out bool valid)
    {
        ...
        foreach (Voiture v in results)
        {
            ...
            if (p <= mx)
            {
                reward[p] = v.AggregatedPositionPoints;
            }
        }
        ...
        foreach (Voiture r in Voitures.Values)
        {
            if (r.RacePosition == 1)
            {
                ...
                for (int a = 0; a < 3; a++)
                {
                    ...
                    if (k > 0)
                    {
                        winnerz.Add(new Tuple<DateTime, int>(f, k));
                    }
                }
            }
        }
    ...
    internal List<int> CircuitKeysForGrandPrix(int gp)
    {
        ...
        foreach (RaceMeeting rm in Races.Values)
        {
            ...
            {
                if (!tracks.Contains(rm.CircuitKey))
                {
                    tracks.Add(rm.CircuitKey);
                }
            }
        }
    ...
    internal int FirstIncompleteSeason()
    {
        ...
        for (int a = ys; a <= yf; a++)
        {
            seasonMeetCount.Add(a, 0);
        }

        foreach (RaceMeeting rm in Instance.Races.Values)
        {
    ...
    internal List<Tuple<int, int, int, int>> CompadresOf(int driverNumber)
    {
                    ...
                    foreach (Voiture w in Voitures.Values)
                    {
                        if (w.RaceMeetingKey == v.RaceMeetingKey)
                        {
                            for (int seat = 0; seat < 3; seat++)
                            {
                                ...
                                if ((p != driverNumber) && (p > 0) && (!myFellowKeys.Contains(p)))
                                {
                                    myFellowKeys.Add(p);
                                }
                            }
        ...
        foreach (int bro in myFellowKeys)
        {
            ...
            if (starts > 0)
            {
                results.Add(new Tuple<int, int, int, int>(starts, wins, losses
                    , bro)); // sortable by number of races and of wins
            }
        }
            ...
            for (int seat = 0; seat < 3; seat++)
            {
                ...
                if (p > 0)
                {
                    ...
                    if (pos != (int) RaceResultConstants.DidNotStart)
                    {
                        ...
                        {
                            pos = 0;
                        } // one of the 'did not finish' constants
                        else
                        {
                            pos = 100 -
                                  pos; // mirroring the RacePosition function, making a lower (but positive) ranking better
                        }

                        if (performances.ContainsKey(qui))
                        {
                            if (pos > performances[qui])
                            {
                                performances[qui] = pos;
                            }
                        }
                        else
                        {
    ...
    internal static string GlyphLiving
    {
        get { return $"{'\x00F6'}"; }
    }

    internal static string GlyphFatalAccident
    {
        get { return $"{'\x0085'}"; }
    }
    ...
    {
        System.Windows.Controls.StackPanel s = new System.Windows.Controls.StackPanel()
        {
            Orientation = System.Windows.Controls.Orientation.Horizontal
            , Margin = new System.Windows.Thickness(6, 0, 6, 0)
        };
        s.Children.Add(new System.Windows.Shapes.Rectangle()
            {Fill = System.Windows.Media.Brushes.Blue, Width = 4, Height = 10});
        s.Children.Add(new System.Windows.Shapes.Rectangle()
            {Fill = System.Windows.Media.Brushes.White, Width = 4, Height = 10});
        s.Children.Add(new System.Windows.Shapes.Rectangle()
            {Fill = System.Windows.Media.Brushes.Red, Width = 4, Height = 10});
        return s;
    }
    ...
    internal void SetChampionshipWinYear(int driverKey, int year)
    {
        if (_firstChampionshipWins.ContainsKey(driverKey))
        {
            int y = _firstChampionshipWins[driverKey];
            if (year < y)
            {
                _firstChampionshipWins[driverKey] = year;
            }
        }
        else
        {
            _firstChampionshipWins.Add(driverKey, year);
        }
    }

    internal int FirstChampionshipWin(int driverKey)
    {
        if (_firstChampionshipWins.ContainsKey(driverKey))
        {
            return _firstChampionshipWins[driverKey];
        }
