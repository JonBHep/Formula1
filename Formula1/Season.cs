...
internal class Season : IComparable<Season>
{
    /// <summary>
        ...
        /// </summary>
        private Dictionary<int, Dictionary<int, float>> _raceConstructorScores = new Dictionary<int, Dictionary<int, float>>();
        ...
        internal int StatedNumberOfRaces { get; set; } // Only needed during a partially completed season so as to calculate which drivers have a chance to win the championship

        internal int Anno { get; set; }

        internal string Specification
        {
            get { return $"{Anno}~{StatedNumberOfRaces}~{PointsAllocationSchemeForDrivers.Specification}~{PointsAllocationSchemeForConstructors.Specification}"; }
            set
            {
        ...
        {
            return this.Anno.CompareTo(other.Anno);
        }
        ...
        private void BuildConstructorsTable()
        {
            ...
            foreach (int raceKey in _raceConstructorScores.Keys)
            {
                ...
                {
                    if (!_constructorScores.ContainsKey(teamkey)) { _constructorScores.Add(teamkey, new Dictionary<int, float>()); }
        ...
        private void BuildResultsTable()
        {
            ...
            foreach (int pisteKey in _raceKeyList)
            {
                ...
                {
                    if (!DriverScoresToday.ContainsKey(k)) { DriverScoresToday.Add(k, 0); }
                }
        ...
        private void BuildDriversTable()
        {
            ...
            foreach (int raceKey in _raceDriverScores.Keys)
            {
                ...
                {
                    if (!_driverScores.ContainsKey(driverkey)) { _driverScores.Add(driverkey, new Dictionary<int, float>()); }
        ...
        private List<int> SeasonRaceKeys()
        {
            ...
            {
                if (rm.RaceDate.Year == Anno) { mtgs.Add(rm); }
            }
        ...
        public void RefreshStatistics()
        {
            ...
            if (StatedNumberOfRaces < _raceKeyList.Count) { StatedNumberOfRaces = _raceKeyList.Count; }
            BuildResultsTable();
            BuildDriversTable();
        ...
        public List<int> SeasonScoringDrivers()
        {
            ...
            foreach (Voiture car in Core.Instance.Voitures.Values)
            {
                ...
                {
                    if (car.PointsForPosition(0).FloatValue > 0) { ScoringDrivers.Add(car.DriverKey(0)); }
                    if (car.PointsForPosition(1).FloatValue > 0) { ScoringDrivers.Add(car.DriverKey(1)); }
                    if (car.PointsForPosition(2).FloatValue > 0) { ScoringDrivers.Add(car.DriverKey(2)); }
                    if (car.PointsForSprintQualifying(0).FloatValue > 0) { ScoringDrivers.Add(car.DriverKey(0)); }
                    if (car.PointsForSprintQualifying(1).FloatValue > 0) { ScoringDrivers.Add(car.DriverKey(1)); }
                    if (car.PointsForSprintQualifying(2).FloatValue > 0) { ScoringDrivers.Add(car.DriverKey(2)); }
                    if (car.PointsForFastestLap(0).FloatValue > 0) { ScoringDrivers.Add(car.DriverKey(0)); }
                    if (car.PointsForFastestLap(1).FloatValue > 0) { ScoringDrivers.Add(car.DriverKey(1)); }
                    if (car.PointsForFastestLap(2).FloatValue > 0) { ScoringDrivers.Add(car.DriverKey(2)); }
                }
            }
            ...
            foreach (int y in ScoringDrivers)
            {
                if (y > 0)
                {
                    if (!uniqueKeys.Contains(y)) { uniqueKeys.Add(y); }
                }
            }
            return uniqueKeys;
        }
        ...
        public List<int> SeasonNonScoringDrivers()
        {
            ...
            foreach (Voiture car in Core.Instance.Voitures.Values)
            {
                if (car.RaceDate.Year == Anno)
                {
                    ...
                    if (a > 0) { if (!FeaturedDrivers.Contains(a)) { FeaturedDrivers.Add(a); } }
                    a = car.DriverKey(1);
                    if (a > 0) { if (!FeaturedDrivers.Contains(a)) { FeaturedDrivers.Add(a); } }
                    a = car.DriverKey(2);
                    if (a > 0) { if (!FeaturedDrivers.Contains(a)) { FeaturedDrivers.Add(a); } }
            ...
            foreach(int b in scorers) { FeaturedDrivers.Remove(b); }
            ...
            foreach (Voiture cr in Core.Instance.Voitures.Values)
            {
                if (cr.RaceDate.Year == Anno)
                {
                    if (Core.Instance.Races[cr.RaceMeetingKey].CountForConstructorsChampionship)
                    {
                        ...
                        {
                            if (!ScoringTeams.Contains(cr.ConstructorKey)) { ScoringTeams.Add(cr.ConstructorKey); }
                        }
                    }
                }
            }
        ...
        {
            if (!Core.Instance.Races[RaceKey].CountForConstructorsChampionship) { return 0; }
            ...
            foreach (Voiture cr in Core.Instance.Voitures.Values)
            {
                if (cr.RaceMeetingKey == RaceKey)
                {
                    ...
                    {
                        float p;
                        if ((Yr==1958)||(Yr==1959)) // in 1958 and 1959 constructors were not entitled to fastest lap points
                        {
                        ...
                        else
                        {
                            ...
                            if (Yr == 1961) { if (p == 9) { p=8; } } // p of 9 for win reduced to 8 (there were no fastest lap points in 1961)
                        }
                        if (cr.ConstructorPointsDisallowed) { p = 0; } // e.g. Brazil GP 1995 two teams points were disallowed for petrol type violation but drivers' points were awarded
                        if (cr.ConstructorPointsPenalty > 0) { p -= cr.ConstructorPointsPenalty; }
                        CarScores.Add(p); 
                    }
                }
            }
            if (Yr < 1979)
            {
                ...
                if (CarScores.Count > 0) { return CarScores.Max(); }
            }
            else
            {
                // From 1979, return all of the 'position points' achieved for that team
                if (CarScores.Count > 0) { return CarScores.Sum(); }
            }
            return 0;
        }

        internal List<int> RaceKeyList { get => _raceKeyList; }
        
        internal float DriverPoints(int RaceKey, int DriverKey) { return _raceDriverScores[RaceKey][DriverKey]; }
        internal float ConstructorPoints(int RaceKey, int ConstructorKey) { return _raceConstructorScores[RaceKey][ConstructorKey]; }
        ...
        internal List<IndividualScore> RaceDriverResults(int RaceKey)
        {
            ...
            foreach (int driverKey in DriverScores.Keys)
            {
                ...
                foreach (int EventKey in _raceKeyList)
                {
                    ...
                    cumul += pts;
                    if (EventKey == RaceKey) { break; } // only count races up to the current one
                }
            ...
            if (statedraces >= soFarRaces.Count)
            {
                ...
                foreach (IndividualScore chap in returnList)
                {
                    ...
                    chap.MaxTheoreticalSeasonTotal = PointsAllocationSchemeForDrivers.ModifiedPointsTotal(driverRealAndPotentialScores);
                }
            }
            returnList.Sort();
            return returnList;
        ...
        internal List<Tuple<float, float,float, int>> RankedDrivers(int RaceKey)
        {
            ...
            {
                rankable.Add(new Tuple<float, float,float, int>(i.CumulativeScoreCounted,i.CumulativeScore,i.MaxTheoreticalSeasonTotal, i.IndividualKey));
            }
            rankable.Sort();
            rankable.Reverse();
            return rankable;
        ...
        internal int DriverSeasonRanked(int position)
        {
            ...
            if (lastRaceRanking.Count > 0) { return lastRaceRanking[position - 1].Item4; } else { return 0; }
        }
        ...
        internal List<IndividualScore> RaceConstructorResults(int RaceKey)
        {
            ...
            foreach (int constructorKey in ConstructorScores.Keys)
            {
                ...
                isc.CumulativeScore = cumul;
                isc.CumulativeScoreCounted =PointsAllocationSchemeForConstructors.ModifiedPointsTotal(constructorScoresToDate);
                returnList.Add(isc);
            }
            returnList.Sort();
            return returnList;
        ...
        internal List<Tuple<int,string>> NumberOfDifferentVictorsInSeason()
        {
            ...
            foreach (int racekey in _raceKeyList)
            {
                ...
                if (drvr > 0)
                {
                    if (Gagnants.ContainsKey(drvr)) { Gagnants[drvr]++; } else { Gagnants.Add(drvr, 1); }
                }
                drvr = winners.Item2;
                if (drvr > 0)
                {
        ...
        internal List< Tuple<int,int, int>> MostWinsInInSeason()
        {
            ...
            foreach (int dd in Gagnants.Keys)
            {
                if (Gagnants[dd] > most) { most = Gagnants[dd]; }
            }
            ...
            foreach (int dd in Gagnants.Keys)
            {
                if (Gagnants[dd] == most)
                {
                    champs.Add(new Tuple<int, int, int>(dd,Gagnants[dd], _raceKeyList.Count)); // driver key, number of wins, races in season
                }
            }
            return champs;
        }
